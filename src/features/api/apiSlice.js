...
import config from '../../config/env'  // Импортируем конфиг
...
export const apiSlice = createApi({
  ...
  baseQuery: fetchBaseQuery({ 
    baseUrl: config.apiUrl  // ✅ Теперь берется из .env
  }),
  tagTypes: [
    'Station', 'Hydropost', 'Synoptic', 'Measurement', 'Bulletins',
    'Observations', 'Telegram', 'Teploenergo', 'SoapObservations',
    'SoapRadiation', 'WmoStation', 'FireDanger', 'NewHydroTelegram',
    'HydroPrecipitation', 'MeteoPrecipitation', 'AvgTemp15Hours',
    'DataById', 'CurrentWeather', 'Forecast3days'
  ],
  endpoints: (builder) => ({
    // ✅ Все URL теперь используют config
    getFireDanger: builder.query({
      query: (reportDate) => 
        `${config.hmcDnrUrl}/fire_dangers/daily_fire_danger?format=json&report_date=${reportDate}`,
      providesTags: ['FireDanger']
    }),
    
    getWmoStations: builder.query({
      query: () => `${config.hmcDnrUrl}/wmo_stations/wmo_stations.json`,
      providesTags: ['WmoStation'],
    }),
    
    getSoapMeteoStations: builder.query({
      query: () => `${config.soapApiUrl}/stations/meteostations?format=json`,
    ...
    getSoapRadiation: builder.query({
      query: (qParams)=>{
        return `${config.soapApiUrl}/observations/observations?min_quality=1&sources=1300&hashes=-1881179977&limit=${qParams.limit}&stations=${qParams.stations}&after=${qParams.notbefore}&before=${qParams.notafter}`
      },
    ...
    getSoapObservations: builder.query({
      query: (qParams) => {
        ...
        return `${config.soapApiUrl}/observations/observations?min_quality=${qParams.quality}&limit=${qParams.limit}&stations=${qParams.stations}&after=${qParams.notbefore}&before=${qParams.notafter}${sources}${hashes}${term}${stream}`
      },
    ...
    getObservations: builder.query({
        ...
        getBulletins: builder.query({
          query: (qParams)=> `${config.hmcDnrUrl}/bulletins/list?format=json&page=${qParams[0]}&bulletin_type=${qParams[1]}`,
          providesTags: ['Bulletins'],
        }),
        forecast3days: builder.query({
          query: reportDate=>`${config.hmcDnrUrl}/bulletins/get_total_forecast?format=json&report_date=${reportDate}`,
        ...
        dataById: builder.query({
          query:id=>`${config.apiUrl}/get?limit=1&nulls=1&lastid=${+id-1}`,
          providesTags: ['DataById']
        }),
        currentWeather: builder.query({
          query:(qParams)=>{
            console.log(`/get?limit=100&stations=${qParams.station}&notbefore=${qParams.notbefore}&notafter=${qParams.notafter}&streams=1&source=10202`)
        ...
        getTemp8: builder.query({
          ...
          query:(currentPage)=>`${config.hmcDnrUrl}/other_observations.json?factor=perc&page_size=15&page=${currentPage}`,
          providesTags: ['Wind', 'Precipitation']
        }),
        getOtherPrecipitation: builder.query({
          query: (monthYear)=> `${config.hmcDnrUrl}/other_observations/monthly_precipitation_data?format=json&month=${monthYear[0]}&year=${monthYear[1]}`,
        ...
        saveHydroData: builder.query({
          query: hydroData=>{
            ...
            return `${config.soapApiUrl}/conservations/conservations?${s.slice(0,-1)}`
          },
        }),
        createStorm: builder.mutation({
          query: bulletin =>({
            method: "POST",
            url: `${config.hmcDnrUrl}/bulletins?format=json`,
            body: bulletin
          })
        })
      })
