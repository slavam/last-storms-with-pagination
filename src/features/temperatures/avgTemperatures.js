...
// [0,3,6,9,12,15,18,21]
    ...
    observations.map((o) => {
      ...
      temps[i][j] = (o.value-273.15).toFixed(1)
      if(o.value) {temps[i][9]+=(+o.value-273.15); temps[i][10]+=1}
    })
    const createTr = (i) => {
  ...
  return (
    <div className="col-md-6 offset-md-3 mt-5">
      ...
      <div className="form-group">
        ...
        <input type="date" id="input-date" name="input-date" value={reportDate} onChange={(event) => setReportDate(event.target.value)} required={true} autoComplete="on" />
      </div>
